...
static NSString * _Nonnull safeStringFromObject(id _Nullable obj, NSString * _Nonnull fallback) {
    ...
    if ([obj respondsToSelector:@selector(string)]) {
        @try {
            id s = safePerform(obj, @selector(string));
            if ([s isKindOfClass:[NSString class]]) return (NSString *)s;
        } @catch (NSException *e) {}
    }
    if ([obj respondsToSelector:@selector(simpleText)]) {
        @try {
            id s = safePerform(obj, @selector(simpleText));
            if ([s isKindOfClass:[NSString class]]) return (NSString *)s;
        } @catch (NSException *e) {}
    }
    @try {
        id runs = safeValueForKey(obj, @"runs");
        if ([runs isKindOfClass:[NSArray class]] && [(NSArray *)runs count] > 0) {
            NSMutableString *combined = [NSMutableString string];
            for (id run in (NSArray *)runs) {
                id text = safeValueForKey(run, @"text");
    ...
    @try {
        ...
        if ([s isKindOfClass:[NSString class]] && s.length > 0 && ![s hasPrefix:@"<"]) return s;
    } @catch (NSException *e) {}
    return fallback ?: @"";
}
...
static UIViewController * _Nullable findBestPresenter(void) {
    ...
    if (vc && vc.view && vc.view.window != nil && !vc.isBeingDismissed) {
        return vc;
    }
    return nil;
}
...
static void presentActionSheetSafely(UIViewController * _Nullable topVC, UIViewController *toPresent, id _Nullable sourceView) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        @try {
            ...
            if (!presenter || presenter.isBeingDismissed || !presenter.view.window) {
                presenter = findBestPresenter();
            }
            if (!presenter) {
                Class ytUIUtils = NSClassFromString(@"YTUIUtils");
                if (ytUIUtils && [ytUIUtils respondsToSelector:@selector(topViewControllerForPresenting)]) {
                    #pragma clang diagnostic push
                    #pragma clang diagnostic ignored "-Warc-performSelector-leaks"
                    UIViewController *cand = [ytUIUtils performSelector:@selector(topViewControllerForPresenting)];
                    #pragma clang diagnostic pop
                    if (cand && cand.view.window != nil && !cand.isBeingDismissed) {
                        presenter = cand;
                    }
                }
            }
...
- (void)showDownloadMenuWithPlayerData:(id)playerData sourceView:(nullable id)sourceView videoID:(NSString *)defaultVideoID {
    ...
    videoID = safeStringFromObject(videoID, self.lastActiveVideoID ?: @"");

    id rawTitle = nil;
    if (activeVideo) {
        @try { rawTitle = [activeVideo valueForKey:@"title"]; } @catch (NSException *e) {}
    }
    if (!rawTitle) {
        @try { rawTitle = [videoDetails valueForKey:@"title"]; } @catch (NSException *e) {}
    }
    if (!rawTitle && activeVC) {
        @try { rawTitle = [activeVC title]; } @catch (NSException *e) {}
    }
    NSString *videoTitle = safeStringFromObject(rawTitle, @"YouTube Video");
    if (videoTitle.length == 0) videoTitle = @"YouTube Video";
...
- (void)showDownloadMenuWithPlayerDataFallback:(id)playerData streamingData:(id)streamingData activeVideo:(id)activeVideo activeVC:(id)activeVC videoTitle:(NSString *)videoTitle videoID:(NSString *)videoID sourceView:(nullable id)sourceView defaultVideoID:(NSString *)defaultVideoID {
    @try {
        videoTitle = safeStringFromObject(videoTitle, @"YouTube Video");
        videoID = safeStringFromObject(videoID, safeStringFromObject(defaultVideoID, self.lastActiveVideoID ?: @""));
        ...
        // 2. Tìm kiếm các luồng Video tương ứng với độ phân giải
        NSMutableArray<NSDictionary *> *videoActions = [NSMutableArray array];
        NSMutableSet *seenKeys = [NSMutableSet set];
        NSArray *standardResolutions = @[@"1080p", @"720p", @"480p", @"360p", @"240p", @"144p"];

        for (NSString *targetRes in standardResolutions) {
            id matchedMP4 = nil;
            for (id format in allVideoFormats) {
                NSString *mime = extractMimeType(format);
                NSString *q = extractQualityLabel(format);
                if ([mime containsString:@"video/mp4"] && [q containsString:targetRes]) {
                    matchedMP4 = format;
                    break;
                }
            }
        ...
        NSLog(@"[YTLite] Exception in showDownloadMenuWithPlayerDataFallback: %@", e);
        showToast(@"Lỗi hiển thị menu tải xuống, vui lòng thử lại!");
    }
}
...
- (void)showHLSDownloadMenuWithVariants:(NSArray<YTLM3U8Variant *> *)variants
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        @try {
            NSString *safeTitle = safeStringFromObject(videoTitle, @"YouTube Video");
            NSString *safeVid = safeStringFromObject(videoID, self.lastActiveVideoID ?: @"");

            UIAlertController *sheet = [UIAlertController alertControllerWithTitle:safeTitle
        ...
        if (safeVid.length > 0) {
            [sheet addAction:[UIAlertAction actionWithTitle:@"🖼️ Tải Ảnh thu nhỏ (Thumbnail HD)" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
                [self downloadThumbnailWithVideoID:safeVid title:safeTitle];
            }]];

            [sheet addAction:[UIAlertAction actionWithTitle:@"📋 Sao chép Liên kết Video" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
                [UIPasteboard generalPasteboard].string = [NSString stringWithFormat:@"https://youtu.be/%@", safeVid];
                showToast(@"Đã sao chép liên kết vào bộ nhớ tạm");
            }]];
        }
        ...
        presentActionSheetSafely(nil, sheet, sourceView);
            ...
            NSLog(@"[YTLite] Exception in showHLSDownloadMenuWithVariants: %@", e);
            showToast(@"Lỗi hiển thị menu HLS, vui lòng thử lại!");
        }
    });
}
